...
var agente_id = 8;
var agente_nombre = "Ventilación y calidad del aire";
var ruta_storage_guardar = '/reporteaire';
...
function datosgenerales()
{
	$.ajax({
		type: "GET",
		dataType: "json",
		url: "/reporteairedatosgenerales/"+proyecto.id+"/"+agente_id+"/"+agente_nombre,
		data:{},
		cache: false,
		success:function(dato)
		{
			...
			if (dato.reporte_ubicacioninstalacion.ubicacionfoto)
			{
				...
				var imagenUrl = '/reporteairemapaubicacion/'+reporteregistro_id+'/'+0;
			...
			if (dato.reporte_responsablesinforme.responsable1)
			{
				...
				var extension = archivo.substring(archivo.lastIndexOf("."));
				var imagenUrl = '/reporteaireresponsabledocumento/'+dato.reporte_responsablesinforme.registro_id+'/'+1+'/'+0;
				...
				var extension = archivo.substring(archivo.lastIndexOf("."));
				var imagenUrl = '/reporteaireresponsabledocumento/'+dato.reporte_responsablesinforme.registro_id+'/'+2+'/'+0;
			...
			menureporte_estado("menureporte_6_2", 1); //Metodo de evaluación
...
//=================================================
//  CARACTERISTICA A EVALUAR DE AIRE 
...
//                 $('#bioaerosoles').prop('checked', caracteristicas.bioaerosoles === 1);
//                 $('#co').prop('checked', caracteristicas.co === 1);
//                 $('#co2').prop('checked', caracteristicas.co2 === 1);
//                 $('#temperatura').prop('checked', caracteristicas.temperatura === 1);
//                 $('#velocidad').prop('checked', caracteristicas.velocidad === 1);
//                 $('#caudal').prop('checked', caracteristicas.caudal === 1);
//                 $('#humedad').prop('checked', caracteristicas.humedad === 1);
...
function tabla_reporte_definiciones(proyecto_id, agente_nombre, reporteregistro_id)
{
	try 
	{
		var ruta = "/reporteairetabladefiniciones/"+proyecto_id+"/"+agente_nombre+"/"+reporteregistro_id;
				...
				function(isConfirm)
				{
					if (isConfirm)
					{
						...
						$.ajax({
							type: "GET",
							dataType: "json",
							url: "/reporteairedefinicioneliminar/"+row.data().id,
							data:{},
							cache: false,
							success:function(dato)
							{
...
{
	window.open('/reporteairemapaubicacion/'+reporteregistro_id+'/'+1);
});
...
function tabla_reporte_categorias(proyecto_id, reporteregistro_id)
{
	try 
	{
		var ruta = "/reporteairecategorias/"+proyecto_id+"/"+reporteregistro_id+"/"+areas_poe;
...
$('#tabla_reporte_categoria tbody').on('click', 'td>button.editar', function()
{
	...
	// Llenar campos
	$('#reporteairecategoria_nombre').val(row.data().reporteairecategoria_nombre);
	$('#reporteairecategoria_total').val(row.data().reporteairecategoria_total);


	if (areas_poe == 1)
	{
		$('#reporteairecategoria_nombre').attr('required', false);
		$('#reporteairecategoria_nombre').attr('disabled', true);

		$('#reporteairecategoria_total').attr('required', false);
		$('#reporteairecategoria_total').attr('disabled', true);
	}
	else
	{
		$('#reporteairecategoria_nombre').attr('disabled', false);
		$('#reporteairecategoria_nombre').attr('required', true);

		$('#reporteairecategoria_total').attr('disabled', false);
		$('#reporteairecategoria_total').attr('required', true);
	}
...
$('#tabla_reporte_categoria tbody').on('click', 'td>button.eliminar', function()
{
	...
	swal({
		title: "¡Confirme que desea eliminar!",
		text: "La categoría: "+row.data().reporteairecategoria_nombre,
		type: "warning",
		showCancelButton: true,
		confirmButtonColor: "#DD6B55",
		confirmButtonText: "Aceptar!",
...
$("#botonguardar_modal_categoria").click(function()
{
	...
	if (valida)
	{
		swal({
			title: "¡Confirme que desea guardar!",
			text: "Categoría: "+$("#reporteairecategoria_nombre").val(),
			type: "warning",
			showCancelButton: true,
			confirmButtonColor: "#DD6B55",
			confirmButtonText: "Guardar!",
...
function tabla_reporte_areas(proyecto_id, reporteregistro_id)
{
	try 
	{
		var ruta = "/reporteaireareas/"+proyecto_id+"/"+reporteregistro_id+"/"+areas_poe;
...
$("#boton_reporte_nuevaarea").click(function()
{
	...
	$.ajax({
		type: "GET",
		dataType: "json",
		url: "/reporteaireareacategorias/"+proyecto.id+"/"+reporteregistro_id+"/0/"+areas_poe,
		data:{},
		cache: false,
		success:function(dato)
		{
...
$('#tabla_reporte_area tbody').on('click', 'td.editar', function()
{
	...
	// Llenar campos
	$('#reporteairearea_instalacion').val(row.data().reporteairearea_instalacion);
	$('#reporteairearea_nombre').val(row.data().reporteairearea_nombre);
	$('#reporteairearea_numorden').val(row.data().reporteairearea_numorden);	
	$('#reporteairearea_porcientooperacion').val(row.data().reporteairearea_porcientooperacion);
	$('#reporteairearea_ventilacionsistema').val(row.data().reporteairearea_ventilacionsistema);
	$('#reporteairearea_ventilacioncaracteristica').val(row.data().reporteairearea_ventilacioncaracteristica);
	$('#reporteairearea_ventilacioncantidad').val(row.data().reporteairearea_ventilacioncantidad);


	if (areas_poe == 1)
	{
		$('#reporteairearea_instalacion').attr('required', false);
		$('#reporteairearea_instalacion').attr('disabled', true);

		$('#reporteairearea_nombre').attr('required', false);
		$('#reporteairearea_nombre').attr('disabled', true);

		$('#reporteairearea_numorden').attr('required', false);
		$('#reporteairearea_numorden').attr('disabled', true);
	}
	else
	{
		$('#reporteairearea_instalacion').attr('disabled', false);
		$('#reporteairearea_instalacion').attr('required', true);

		$('#reporteairearea_nombre').attr('disabled', false);
		$('#reporteairearea_nombre').attr('required', true);

		$('#reporteairearea_numorden').attr('disabled', false);
		$('#reporteairearea_numorden').attr('required', true);
	}
	...
	$.ajax({
		type: "GET",
		dataType: "json",
		url: "/reporteaireareacategorias/"+proyecto.id+"/"+reporteregistro_id+"/"+row.data().id+"/"+areas_poe,
		data:{},
		cache: false,
		success:function(dato)
		{
...
$('#tabla_reporte_area tbody').on('click', 'td>button.eliminar', function()
{
	...
	swal({
		title: "¡Confirme que desea eliminar!",
		text: "El área: "+row.data().reporteairearea_nombre,
		type: "warning",
		showCancelButton: true,
		confirmButtonColor: "#DD6B55",
		confirmButtonText: "Aceptar!",
			...
			function(isConfirm)
			{
				if (isConfirm)
				{
					...
					$.ajax({
						type: "GET",
						dataType: "json",
						url: "/reporteaireareaeliminar/"+row.data().id,
						data:{},
						cache: false,
						success:function(dato)
						{
...
$("#botonguardar_modal_area").click(function()
{
	...
	if (valida)
	{
		swal({
			title: "¡Confirme que desea guardar!",
			text: "Área: "+$("#reporteairearea_nombre").val(),
			type: "warning",
			showCancelButton: true,
			confirmButtonColor: "#DD6B55",
			confirmButtonText: "Guardar!",
...
function tabla_reporte_5_5(tbody)
{
	...
	datatable_reporte_5_5 = $('#tabla_reporte_5_5').DataTable({
		"lengthMenu": [[20, 50, 100, -1], [20, 50, 100, "Todos"]],
		"rowsGroup": [1, 0, 2, 3, 4, 5], //agrupar filas
		"order": [[ 0, "DESC" ]],
		"ordering": false,
		"searching": true,
		"processing": true,
...
const idsMenureporte = ["menureporte_7_1", "menureporte_7_2", "menureporte_7_3", "menureporte_7_4", "menureporte_7_5", "menureporte_7_6", "menureporte_7_7"];

const estadosMenureporte = idsMenureporte.reduce((acc, id) => ({ ...acc, [id]: 0 }), {});

function actualizarMenureporteEstado(id, nuevoEstado) {
    if (estadosMenureporte[id] !== 1 || nuevoEstado !== 0) {
        menureporte_estado(id, nuevoEstado);
        estadosMenureporte[id] = nuevoEstado; 
    }
}
        ...
        if (datatable_reporte_puntos != null) {
            ...
            datatable_reporte_puntos = $('#tabla_reporte_puntos').DataTable({
                ajax: {
                    ...
                    data: {},
                    dataSrc: function (json) {
                        idsMenureporte.forEach((id, index) => {
                            const tablaReporte = json[`tabla_reporte_${index + 1}`];
                ...
                columns: [
                    { data: "reporteaireevaluacion_punto", defaultContent: "-", orderable: false },
                    { data: "reporteairearea_instalacion", defaultContent: "-", orderable: false },
                    { data: "reporteairearea_nombre", defaultContent: "-", orderable: false },
                    { data: "reporteaireevaluacion_ubicacion", defaultContent: "-", orderable: false },
                    { data: "boton_editar", defaultContent: "-", className: 'editar', orderable: false },
                    { data: "boton_eliminar", defaultContent: "-", orderable: false }
                ],
                lengthMenu: [[20, 50, 100, -1], [20, 50, 100, "Todos"]],
                rowsGroup: [1, 2, 0],
                ordering: false,
...
$("#boton_reporte_nuevopuntomedicion").click(function()
{
	...
	// Campos Hidden
	$('#reporteaireevaluacion_id').val(0);


	// Campo select areas
	$('#reporteaireevaluacion_area_id').html(selectareas);
	...
	// Vaciar tabla evaluacion categorias
	tabla_evaluacion_categorias(null);
});


function aireevaluacion_categorias(reporteaireevaluacion_id, reporteairearea_id)
{
	if (parseInt(reporteairearea_id) > 0)
	{
		// Consultar categorias
		$.ajax({
			type: "GET",
			dataType: "json",
			url: "/reporteaireevaluacioncategorias/"+reporteaireevaluacion_id+"/"+reporteairearea_id+"/"+areas_poe,
			data:{},
			cache: false,
			success:function(dato)
			{
			...
			beforeSend: function()
			{
				$('#tabla_evaluacion_categorias tbody').html('<tr><td colspan="4" style="text-align: center;"><i class="fa fa-spin fa-spinner" style="font-size: 40px!important;"></i></td></tr>');
			},
			error: function(dato)
			{
				$('#tabla_evaluacion_categorias tbody').html('<tr><td colspan="4" style="text-align: center;">Error al cargar las categorías</td></tr>');
				return false;
			}
		});//Fin ajax
	}
...
$('#tabla_reporte_puntos tbody').on('click', 'td.editar', function()
{
	...
	// Campos Hidden
	$('#reporteaireevaluacion_id').val(row.data().id);
	...
	// Llenar campos
	$('#reporteaireevaluacion_punto').val(row.data().reporteaireevaluacion_punto);
	$('#reporteaireevaluacion_area_id').val(row.data().reporteairearea_id);
	$('#reporteaireevaluacion_ubicacion').val(row.data().reporteaireevaluacion_ubicacion);
	$('#reporteaireevaluacion_ct').val(row.data().reporteaireevaluacion_ct);
	$('#reporteaireevaluacion_ctma').val(row.data().reporteaireevaluacion_ctma);
	$('#reporteaireevaluacion_hongos').val(row.data().reporteaireevaluacion_hongos);
	$('#reporteaireevaluacion_levaduras').val(row.data().reporteaireevaluacion_levaduras);
	$('#reporteaireevaluacion_temperatura').val(row.data().reporteaireevaluacion_temperatura);
	$('#reporteaireevaluacion_velocidad').val(row.data().reporteaireevaluacion_velocidad);
	$('#reporteaireevaluacion_velocidadlimite').val(row.data().reporteaireevaluacion_velocidadlimite);
	...
	// Consultar categorias
	aireevaluacion_categorias(row.data().id, row.data().reporteairearea_id);
});
...
$('#tabla_reporte_puntos tbody').on('click', 'td>button.eliminar', function()
{
	...
	swal({
		title: "¡Confirme que desea eliminar!",
		text: "Punto "+row.data().reporteaireevaluacion_punto+" de medición de aire",
		type: "warning",
		showCancelButton: true,
		confirmButtonColor: "#DD6B55",
		confirmButtonText: "Aceptar!",
			...
			function(isConfirm)
			{
				if (isConfirm)
				{
					...
					$.ajax({
						type: "GET",
						dataType: "json",
						url: "/reporteaireevaluacioneliminar/"+row.data().id,
						data:{},
						cache: false,
						success:function(dato)
						{
...
$("#botonguardar_modal_puntomedicion").click(function()
{
	...
	if (valida)
	{
		swal({
			title: "¡Confirme que desea guardar!",
			text: "Punto de medición de aire",
			type: "warning",
			showCancelButton: true,
			confirmButtonColor: "#DD6B55",
			confirmButtonText: "Guardar!",
...
function tabla_reporte_7_1(tbody)
{
	...
	datatable_reporte_7_1 = $('#tabla_reporte_7_1').DataTable({
		...
		rowCallback: function(row, data, index)
		{
			...
			if($(row).find('td:eq(9)').text() == "Dentro de norma")
			{
...
function tabla_reporte_7_2(tbody)
{
	if (datatable_reporte_7_2 != null)
	{
		datatable_reporte_7_2.destroy();
	}

	$('#tabla_reporte_7_2 tbody').html(tbody);
...
var datatable_reporte_7_4 = null;
function tabla_reporte_7_4(tbody)
{
	if (datatable_reporte_7_4 != null)
	{
		datatable_reporte_7_4.destroy();
	}

	$('#tabla_reporte_7_4 tbody').html(tbody);


	datatable_reporte_7_4 = $('#tabla_reporte_7_4').DataTable({
		lengthMenu: [[20, 50, 100, -1], [20, 50, 100, "Todos"]],
		rowsGroup: [1, 2, 3, 4, 0], //agrupar filas
		order: [[ 0, "DESC" ]],
		ordering: false,
...
function tabla_reporte_7_5(tbody)
{
	...
	$('#tabla_reporte_7_5 tbody').html(tbody);
...
function tabla_reporte_7_6(tbody)
{
	if (datatable_reporte_7_6 != null)
	{
		datatable_reporte_7_6.destroy();
	}

	$('#tabla_reporte_7_6 tbody').html(tbody);
...
function tabla_reporte_matriz(proyecto_id, reporteregistro_id)
{
	try 
	{
		var ruta = "/reporteairematriztabla/"+proyecto_id+"/"+reporteregistro_id+"/"+areas_poe;
			...
			else
			{
				$('#tabla_reporte_matriz').html('<thead>'+
													'<tr>'+
														...
														'<th rowspan="2">Número de<br>personas<br>en el área</th>'+
														'<th rowspan="2">Grupo de<br>exposición<br>homogénea</th>'+
														'<th colspan="9">Ventilación y calidad del aire</th>'+
													'</tr>'+
													'<tr>'+
														'<th width="4%">Temperatura<br>(°C)</th>'+
														'<th width="4%">Velocidad<br>del aire (m/s)</th>'+
														'<th width="4%">Humedad<br>relativa (%)</th>'+
														'<th width="4%">CO<br>(ppm)</th>'+
														'<th width="4%">CO<sub>2</sub><br>(ppm)</th>'+
														'<th width="4%">CT<br>(UFC)</th>'+
														'<th width="4%">CTMA<br>(UFC)</th>'+
														'<th width="4%">Hongos<br>(UFC)</th>'+
														'<th width="4%">Levaduras<br>(UFC)</th>'+
													'</tr>'+
												'</thead>'+
												'<tbody></tbody>');
				...
				datatable_reporte_matriz = $('#tabla_reporte_matriz').DataTable({
					...
					columns: [
						...
						{
							data: "reporteaireevaluacioncategorias_nombre",
							defaultContent: "-",
							orderable: false,
						},
						{
							data: "reporteaireevaluacioncategorias_ficha",
							defaultContent: "-",
							orderable: true,
						},
						{
						...
						{
							data: "reporteaireevaluacioncategorias_geo",
							defaultContent: "-",
							orderable: false,
						},
						{
							data: "reporteaireevaluacion_temperatura",
							defaultContent: "-",
							orderable: false,
						},
						{
							data: "reporteaireevaluacion_velocidad",
							defaultContent: "-",
							orderable: false,
						},
						{
							data: "reporteaireevaluacion_humedad",
							defaultContent: "-",
							orderable: false,
						},
						{
							data: "reporteaireevaluacion_co",
							defaultContent: "-",
							orderable: false,
						},
						{
							data: "reporteaireevaluacion_co2",
							defaultContent: "-",
							orderable: false,
						},
						{
							data: "reporteaireevaluacion_ct",
							defaultContent: "-",
							orderable: false,
						},
						{
							data: "reporteaireevaluacion_ctma",
							defaultContent: "-",
							orderable: false,
						},
						{
							data: "reporteaireevaluacion_hongos",
							defaultContent: "-",
							orderable: false,
						},
						{
							data: "reporteaireevaluacion_levaduras",
							defaultContent: "-",
							orderable: false,
						}
					],
					lengthMenu: [[20, 50, 100, -1], [20, 50, 100, "Todos"]],
					rowsGroup: [1, 2, 3, 0, 4, 10, 11, 12, 13, 14, 15, 16, 17, 18], //agrupar filas
					order: [[ 0, "ASC" ]],
					ordering: false,
					processing: true,
					searching: true,
		...
		else if (parseInt(proyecto.catregion_id) == 2) //REGION SUR
		{
			...
			else
			{
				...
				datatable_reporte_matriz = $('#tabla_reporte_matriz').DataTable({
					...
					lengthMenu: [[20, 50, 100, -1], [20, 50, 100, "Todos"]],
					rowsGroup: [1, 2, 3, 0, 4, 8, 9, 10, 11, 12, 13, 14, 15, 16], //agrupar filas
					order: [[ 0, "ASC" ]],
					ordering: false,
					processing: true,
					searching: true,
		...
		else  // REGION MARINA
		{
			...
			$('#tabla_reporte_matriz').html('<thead>'+
													'<tr>'+
														'<th rowspan="2">Contador</th>'+
														'<th rowspan="2">Subdirección o<br>corporativo</th>'+
														'<th rowspan="2">Coordinación</th>'+
														'<th rowspan="2">Instalación</th>'+
														'<th rowspan="2">Nombre</th>'+
														'<th rowspan="2">Ficha</th>'+
														'<th rowspan="2">Categoría</th>'+
														'<th colspan="6">Ventilación y calidad del aire</th>'+
													'</tr>'+
													'<tr>'+
														'<th width="4%">Temperatura<br>(°C)</th>'+
														'<th width="4%">Velocidad<br>del aire (m/s)</th>'+
														...
														'<th width="4%">CO<br>(ppm)</th>'+
														'<th width="4%">CO<sub>2</sub><br>(ppm)</th>'+
														'<th width="11%">Bioaerosoles<br>"CT, CTMA, hongos, levaduras"<br>(UFC/mtra)</th>'+
													'</tr>'+
												'</thead>'+
												'<tbody></tbody>');
			...
			$.ajax({
				...
				success:function(dato)
				{
					...
					datatable_reporte_matriz = $('#tabla_reporte_matriz').DataTable({
						lengthMenu: [[20, 50, 100, -1], [20, 50, 100, "Todos"]],
						rowsGroup: [1, 2, 3, 0, 4, 7, 8, 9, 10, 11, 12], //agrupar filas
						order: [[ 0, "DESC" ]],
						ordering: false,
						searching: true,
						processing: true,
...
function reporte_dashboard(proyecto_id, reporteregistro_id)
{
	...
	$.ajax({
		type: "GET",
		dataType: "json",
		url: "/reporteairedashboard/"+proyecto_id+"/"+reporteregistro_id+"/"+areas_poe,
		data:{},
		cache: false,
		success:function(dato)
		{
			...
			$("#dashboard_temperatura").html(dato.dashboard_temperatura);
			$("#dashboard_velocidad").html(dato.dashboard_velocidad);
			$("#dashboard_humedad").html(dato.dashboard_humedad);
			$("#dashboard_co").html(dato.dashboard_co);
...
function tabla_reporte_recomendaciones(proyecto_id, reporteregistro_id, agente_nombre)
{
	try 
	{
		var ruta = "/reporteairerecomendacionestabla/"+proyecto_id+"/"+reporteregistro_id+"/"+agente_nombre;
...
$("#botonguardar_reporte_recomendaciones").click(function()
{
	...
	// datatable_recomendaciones.search($(this).val()).draw();
...
{
	window.open('/reporteaireresponsabledocumento/'+reporteregistro_id+'/'+1+'/'+1);
});


$("#boton_descargarresponsabledoc2").click(function()
{
	window.open('/reporteaireresponsabledocumento/'+reporteregistro_id+'/'+2+'/'+1);
});
...
function tabla_reporte_planos(proyecto_id, reporteregistro_id, agente_nombre)
{
	try 
	{
		var ruta = "/reporteaireplanostabla/"+proyecto_id+"/"+reporteregistro_id+"/"+agente_nombre;
...
function tabla_reporte_equipoutilizado(proyecto_id, reporteregistro_id, agente_nombre)
{
	try 
	{
		var ruta = "/reporteaireequipoutilizadotabla/"+proyecto_id+"/"+reporteregistro_id+"/"+agente_nombre;
...
function tabla_reporte_anexosresultados(proyecto_id, reporteregistro_id, agente_nombre)
{
	try 
	{
		var ruta = "/reporteaireanexosresultadostabla/"+proyecto_id+"/"+reporteregistro_id+"/"+agente_nombre;
...
function tabla_reporte_acreditacionaprobacion(proyecto_id, reporteregistro_id, agente_nombre)
{
	try 
	{
		var ruta = "/reporteaireanexosacreditacionestabla/"+proyecto_id+"/"+reporteregistro_id+"/"+agente_nombre;
...
//=================================================
// ANEXOS - NOTAS ACLARATORIAS (STPS y EMA)
...
var datatable_notas_stps = null;
function tabla_notas_stps(proyecto_id, reporteregistro_id, agente_nombre)
{
	try 
	{
		var ruta = "/reporteairenotasstpstabla/"+proyecto_id+"/"+reporteregistro_id+"/"+agente_nombre;

		if (datatable_notas_stps != null)
		{
			datatable_notas_stps.clear().draw();
			datatable_notas_stps.ajax.url(ruta).load();
		}
		else
		{
			var numeroejecucion = 1;
			datatable_notas_stps = $('#tabla_notas_stps').DataTable({
				ajax: {
					url: ruta,
					type: "get",
					cache: false,
					...
					error: function (xhr, error, code)
					{						
						console.log('error en datatable_notas_stps '+code);
						if (numeroejecucion <= 1)
						{
		...
		// Tooltip en DataTable
		datatable_notas_stps.on('draw', function ()
		{
			$('[data-toggle="tooltip"]').tooltip();
		});
	}
...
var datatable_notas_ema = null;
function tabla_notas_ema(proyecto_id, reporteregistro_id, agente_nombre)
{
	try 
	{
		var ruta = "/reporteairenotasematabla/"+proyecto_id+"/"+reporteregistro_id+"/"+agente_nombre;

		if (datatable_notas_ema != null)
		{
			datatable_notas_ema.clear().draw();
			datatable_notas_ema.ajax.url(ruta).load();
		}
		else
		{
			var numeroejecucion = 1;
			datatable_notas_ema = $('#tabla_notas_ema').DataTable({
				ajax: {
					url: ruta,
					type: "get",
					cache: false,
					...
					error: function (xhr, error, code)
					{						
						console.log('error en datatable_notas_ema '+code);
						if (numeroejecucion <= 1)
						{
		...
		// Tooltip en DataTable
		datatable_notas_ema.on('draw', function ()
		{
			$('[data-toggle="tooltip"]').tooltip();
		});
	}
...
$(".boton_nuevanota").click(function()
{
	$('#form_modal_notas').each(function(){
		this.reset();
	});


	// Campos Hidden
	$('#reportenotas_id').val(0);
	$('#reportenotas_tipo').val(parseInt($(this).val()));


	// Tipo
	if (parseInt($(this).val()) == 1)
	{
		$('#reportenotas_descripcion').val('El siguiente apartado contiene la aprobación del laboratorio prueba denominado (RAZÓN_SOCIAL_DEL_LABORATORIO) ante la STPS del parámetro (NOMBRE_PARÁMETRO), bajo el método (NOMBRE_DEL_MÉTODO).');

		$('#modal_reporte_notas .modal-title').html('Nota aclaratoria STPS');
	}
	else
	{
		$('#reportenotas_descripcion').val('El siguiente apartado contiene la acreditación del laboratorio prueba denominado (RAZÓN_SOCIAL_DEL_LABORATORIO) ante la entidad mexicana de acreditación (ema) del parámetro (NOMBRE_PARÁMETRO), bajo el método (NOMBRE_DEL_MÉTODO).');

		$('#modal_reporte_notas .modal-title').html('Nota aclaratoria EMA');
	}


	// mostrar modal
	$('#modal_reporte_notas').modal({backdrop:false});
});


$('#tabla_notas_stps tbody').on('click', 'td.editar', function()
{
	var tr = $(this).closest('tr');
	var row = datatable_notas_stps.row(tr);
	...
	// Campos Hidden
	$('#reportenotas_id').val(row.data().id);
	$('#reportenotas_tipo').val(1);


	// Llenar campos
	$('#reportenotas_descripcion').val(row.data().reportenotas_descripcion);
...
$('#tabla_notas_ema tbody').on('click', 'td.editar', function()
{
	var tr = $(this).closest('tr');
	var row = datatable_notas_ema.row(tr);
...
$('#tabla_notas_stps tbody').on('click', 'td>button.eliminar', function()
{
	var tr = $(this).closest('tr');
	var row = datatable_notas_stps.row(tr);
			...
			function(isConfirm)
			{
				if (isConfirm)
				{
					...
					$.ajax({
						type: "GET",
						dataType: "json",
						url: "/reporteairenotaseliminar/"+row.data().id,
						data:{},
						cache: false,
						success:function(dato)
						{
...
$('#tabla_notas_ema tbody').on('click', 'td>button.eliminar', function()
{
	var tr = $(this).closest('tr');
	var row = datatable_notas_ema.row(tr);
...
$("#botonguardar_modal_notas").click(function()
{
	// valida campos vacios
	var valida = this.form.checkValidity();
	if (valida)
			...
			if (isConfirm)
			{
				...
				$('#form_modal_notas').ajaxForm({
					...
					success: function(dato)
					{
						...
						// actualiza boton
						$('#botonguardar_modal_notas').html('Guardar <i class="fa fa-save"></i>');
						$('#botonguardar_modal_notas').attr('disabled', false);


						// cerrar modal
						$('#modal_reporte_notas').modal('hide');
					},
					beforeSend: function()
					{
						$('#botonguardar_modal_notas').html('Guardando <i class="fa fa-spin fa-spinner"></i>');
						$('#botonguardar_modal_notas').attr('disabled', true);
					},
					error: function(dato)
					{
						// actualiza boton
...
function tabla_reporte_revisiones(proyecto_id)
{
	try 
	{
		var ruta = "/reporteairerevisionestabla/"+proyecto_id;
			...
			function(isConfirm)
			{
				if (isConfirm)
				{
					...
					$.ajax({
						type: "GET",
						dataType: "json",
						url: "/reporteairerevisionconcluir/"+reporte_id,
						data:{},
						cache: false,
						success:function(dato)
						{
			...
			html2canvas(document.querySelector("#tabla_dashboard"), { scale: 6}).then(canvas =>
			{
				...
				setTimeout(function()
				{
					...
					$.ajax({
						...
						success:function(dato)
						{
							ventana = window.open('/reporteaireworddescargar/'+proyecto.id+"/"+row.data().id+"/"+ultimarevision_id);
