...
var Tablarecocategoriaspsico = null;
var Tablarecoareaspsico = null;

ID_RECOPSICOCATEGORIA = 0;
ID_RECOPSICOAREA = 0;
...
$('.multisteps-form__progress-btn').click(function () {
	switch (this.id) {
	  case "steps_menu_tab2":
            ...
            mostrartablarecocategoriaspsico();
            break;
		case "steps_menu_tab3":
			recsensorial = $("#recsensorial_id").val(); 
            mostrartablarecoareaspsico();
		break;
...
$("#boton_descargarmapaubicacion").click(function () {
	window.open('/mostrarmapapsico/1/' + recsensorial);
});

$("#boton_descargarplanoinstalacion").click(function () {
	window.open('/mostrarplanopsico/1/' + recsensorial);
});

$("#boton_descargarfotoinstalacion").click(function () {
	window.open('/mostrarfotoinstalacionpsico/1/' + recsensorial);
});
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            $.ajax({
                ...
                url: '/sincronizarHigienePsico',
                type: 'POST',

                data: {
                    RECPSICO_ID: recsensorial,
...
$("#boton_guardar_categoria").click(function (e) {
    ...
    if (formularioValido) {
        ...
        $(".generarturnos ").each(function() {
         ...
         const requestData = {
            api: 1,
            ID_RECOPSICOCATEGORIA: ID_RECOPSICOCATEGORIA,
    ...
    if (ID_RECOPSICOCATEGORIA == 0) {
        ...
        alertMensajeConfirm({
            ...
            await ajaxAwaitFormData(requestData, 'recopsicocategoria', 'form_categoria', 'boton_guardar_categoria', { callbackAfter: true, callbackBefore: true }, () => {
                ...
                $('.swal2-popup').addClass('ld ld-breath')
                    ...
                    ID_RECOPSICOCATEGORIA = data.categorias.ID_RECOPSICOCATEGORIA
                    alertMensaje('success','Información guardada correctamente', 'Esta información esta lista para usarse',null,null, 1500)
                     $('#modal_categoria').modal('hide')
                    document.getElementById('form_categoria').reset();
                    Tablarecocategoriaspsico.ajax.reload()
...
function mostrartablarecocategoriaspsico() {
	try {
		var ruta = "/Tablarecocategoriaspsico";

		if (Tablarecocategoriaspsico != null) {
			Tablarecocategoriaspsico.destroy();
		}

		Tablarecocategoriaspsico = $('#Tablarecocategoriaspsico').DataTable({
			"ajax": {
				"url": ruta,
				"type": "get",
				"cache": false,
				"data": {
					psicoid: recsensorial 
				},
				"error": function (xhr, error, code) {
					console.log('error en Tablarecocategoriaspsico');
				}
			},
            "columns": [
                {
		...
		Tablarecocategoriaspsico.on('draw', function () {
			$('[data-toggle="tooltip"]').tooltip();
		});

	} catch (exception) {
		console.error("Error en Tablarecocategoriaspsico:", exception);
	}
}

$('#Tablarecocategoriaspsico tbody').on('click', 'td>button.editar', function () {
    var tr = $(this).closest('tr');
    var row = Tablarecocategoriaspsico.row(tr);


    ID_RECOPSICOCATEGORIA = row.data().ID_RECOPSICOCATEGORIA;

    editarDatoTabla(row.data(), 'form_categoria', 'modal_categoria',1);


    $('#modal_categoria .modal-title').html(row.data().RECPSICO_NOMBRECATEGORIA);
...
$("#boton_guardar_area").click(function (e) {
    ...
    if (ID_RECOPSICOAREA == 0) {
        ...
        alertMensajeConfirm({
            ...
            await ajaxAwaitFormData({ api: 1,RECPSICO_ID: recsensorial, ID_RECOPSICOAREA: ID_RECOPSICOAREA }, 'recopsicoarea', 'form_area', 'boton_guardar_area', { callbackAfter: true, callbackBefore: true }, () => {
                ...
                $('.swal2-popup').addClass('ld ld-breath')
                    ...
                    ID_RECOPSICOAREA = data.areas.ID_RECOPSICOAREA
                    alertMensaje('success','Información guardada correctamente', 'Esta información esta lista para usarse',null,null, 1500)
                     $('#modal_area').modal('hide')
                    document.getElementById('form_area').reset();
                    Tablarecoareaspsico.ajax.reload()
...
function mostrartablarecoareaspsico() {
	try {
		var ruta = "/Tablarecoareaspsico";

		if (Tablarecoareaspsico != null) {
			Tablarecoareaspsico.destroy();
		}

		Tablarecoareaspsico = $('#Tablarecoareaspsico').DataTable({
			"ajax": {
				"url": ruta,
				"type": "get",
				"cache": false,
				...
				"error": function (xhr, error, code) {
					console.log('error en Tablarecoareaspsico');
				}
			},
			"columns": [
				{
				...
				{
					"data": "RECPSICOAREA_NOMBRE",
					"defaultContent": "-"
				},
				{
					"data": "RECPSICOAREA_PROCESO",
					"defaultContent": "-"
				},
				{ 
					data: 'BTN_EDITAR',
		...
		Tablarecoareaspsico.on('draw', function () {
			$('[data-toggle="tooltip"]').tooltip();
		});

	} catch (exception) {
		console.error("Error en Tablarecoareaspsico:", exception);
	}
}

$('#Tablarecoareaspsico tbody').on('click', 'td>button.editar', function () {
    var tr = $(this).closest('tr');
    var row = Tablarecoareaspsico.row(tr);

    ID_RECOPSICOAREA = row.data().ID_RECOPSICOAREA;

    editarDatoTabla(row.data(), 'form_area', 'modal_area',1);

    $('#modal_area .modal-title').html(row.data().RECPSICOAREA_NOMBRE);
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (response && response.length > 0) {
                ...
                // IMPORTANTE: cargar sin recalcular
                validarEmpleados(true);
                ...
                // Totales
                document.getElementById('RECPSICO_TOTALHOMBRESSELECCION').value =
                    data.RECPSICO_TOTALHOMBRESSELECCION || 0;

                document.getElementById('RECPSICO_TOTALMUJERESSELECCION').value =
                    data.RECPSICO_TOTALMUJERESSELECCION || 0;

                document.getElementById('RECPSICO_PORCENTAJEHOMBRESTRABAJO').value =
                    data.RECPSICO_PORCENTAJEHOMBRESTRABAJO || 0;

                document.getElementById('RECPSICO_PORCENTAJEMUJERESTRABAJO').value =
                    data.RECPSICO_PORCENTAJEMUJERESTRABAJO || 0;


                // Recuperar guías guardadas
                document.getElementById('option1').checked = data.RECPSICO_GUIAI == 1;
                document.getElementById('option2').checked = data.RECPSICO_GUIAII == 1;
                document.getElementById('option3').checked = data.RECPSICO_GUIAIII == 1;
                document.getElementById('option4').checked = data.RECPSICO_GUIAV == 1;


                // Si hay guías adicionales
                if (
                    data.RECPSICO_GUIAII == 1 ||
                    data.RECPSICO_GUIAIII == 1 ||
                    data.RECPSICO_GUIAV == 1
                ) {
...
//                     document.getElementById('campos_opcionales').style.display = 'block';
//                 }
...
// function validarEmpleados(cargando = false) {
...
// 		// ESTO SIEMPRE
// 		document.getElementById('campos_opcionales').style.display = 'none';
// 		document.getElementById('generos_container').style.display = 'none';
// 		document.getElementById('resultados_trabajadores').style.display = 'none';
...
function validarEmpleados(cargando = false) {
    ...
    // SIEMPRE ocultar y reiniciar opciones adicionales
    document.getElementById('habilitar_opcional').checked = false;
    ...
    document.getElementById('tipo_valor_hombres').disabled = true;
    document.getElementById('valor_hombres').disabled = true;
    document.getElementById('valor_mujeres').disabled = true;
    ...
    // Llenar opciones del select Aplicable a
    selectAplicableA.innerHTML = '<option value="">Selecciona una opción</option>';
    ...
    if (!isNaN(totalEmpleados) && totalEmpleados > 0) {
        ...
        if (totalEmpleados <= 50) {
            ...
            selectAplicableA.innerHTML +=
                '<option value="opcion1">TODOS LOS TRABAJADORES</option>';
            ...
            selectAplicableA.innerHTML +=
                '<option value="opcion2">MUESTRA DE TRABAJADORES</option>';
        }
